...
def get_config():
    ...
    print('*' * (dis_lenth + 20))
    print(' ' * (dis_lenth + 4) + 'POWERED BY GTLWIFI')
    while(1):
    ...
    if password == '':
        password = 'zzz'
    client = SCPCli(net_add, int(port), username, password)
    ...
    while(1):
        try:
            while(1):
                ...
                mode = str(mode)
                switch = {
                    '1': [op_wireless, client],
                    '2': [op_drcom, client],
                    '3': [op_passwd, client],
                    '4': [op_reboot, client]
                    }
                run_func = switch[mode][0]
                run_func(switch[mode][1])
                # 执行完成
                mode = input(
