...
def get_config():
    ...
    while(1):
        net_add = input('请输入网络地址(默认: 192.168.11.2) : ')    # 获取网络地址
        if net_add == '':
...
def mode_select(dis_lenth):
    ...
    print(' |' + ' ' * int((dis_lenth - 4) / 2) + '|' + ' ' * int((dis_lenth - 4) / 2) + '|')
    print(' |' + '{:^21}'.format('1. SSH发送指令(不完善)') + '|' + '{:^24}'.format('2. SCP上传文件') + '|')
    print(' |' + ' ' * int((dis_lenth - 4) / 2) + '|' + ' ' * int((dis_lenth - 4) / 2) + '|')
    print(' +' + '-' * int((dis_lenth - 4) / 2) + '+' + '-' * int((dis_lenth - 4) / 2) + '+')
    print(' |' + ' ' * int((dis_lenth - 4) / 2) + '|' + ' ' * int((dis_lenth - 4) / 2) + '|')
    ...
    if mode == 4:
        ...
        print(' +' + '-' * int((dis_lenth - 4) / 2) + '+' + '-' * int((dis_lenth - 4) / 2) + '+')
        print(' |' + ' ' * int((dis_lenth - 4) / 2) + '|' + ' ' * int((dis_lenth - 4) / 2) + '|')
        print(' |' + '{:^24}'.format('3. 重启系统') + '|' + '{:^22}'.format('4. 登录密码修改') + '|')
        print(' |' + ' ' * int((dis_lenth - 4) / 2) + '|' + ' ' * int((dis_lenth - 4) / 2) + '|')
        print(' +' + '-' * int((dis_lenth - 4) / 2) + '+' + '-' * int((dis_lenth - 4) / 2) + '+')
        while(1):
            try:
...
def exec_command_printing_result():
    while(1):
        print(client.exec_command(), end = '')

#def exec_command_send_command():
#    while(1):
#        cmd = input('') + '\n'
...
def exec_command(para):
    try:
        cmd = ''
        #send = threading.Thread(target = exec_command_send_command)
        print = threading.Thread(target = exec_command_printing_result)
        #send.start()
        print.start()
        while(1):
            cmd = input('') + '\n'
